...
void onmessage(int fd, const unsigned char *msg, uint64_t size, int type)
{
	...
	if (strcmp((char *)msg, "qtApp") == 0)
	{
		...
		printf("fw: %s\r\n", firmware_GW_version);
		printf("Project id %s\r\n", "Sitech Co.Ltd");
		int data_len = sprintf(data, TEMPLATE_HARDWARE_ID, CMD_SERVER_TO_QT_HARDWARE_ID, 11, mac_final, 15, firmware_GW_version, 13, "Sitech Co.Ltd");
		// printf("reach here\r\n");
...
void updateServerSchedule(time_active_t *time_active)
{
	...
	// printf("\r\nmode cross is: %d", time_active_test.hh_start);
